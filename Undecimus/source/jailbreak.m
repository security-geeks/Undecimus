...
#undef cache_address
    ...
    
    {
        ...
        _assert(ensure_directory("/Library/Caches", root_pw->pw_uid, S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO), localize(@"Unable to repair caches directory for %@.", substitutor->name), true);
    ...
    
    {
        ...
        if (prefs->load_tweaks) {
            ...
            _assert(create_file(substitutor->loader_killswitch, root_pw->pw_uid, 644), localize(@"Unable to disable %@'s loader.", substitutor->name), true);
        }
        ...
        if (access("/Library/substrate", F_OK) == ERR_SUCCESS &&
            ...
            _assert(clean_file(substitutor->bootstrap_tools), localize(@"Unable to clean old %@ bootstrap tools directory.", substitutor->name), true);
            _assert([fileManager moveItemAtPath:@"/Library/substrate" toPath:@(substitutor->bootstrap_tools) error:nil], localize(@"Unable to move %@ bootstrap tools directory.", substitutor->name), true);
        }
        _assert(runCommand(substitutor->startup_executable, NULL) == ERR_SUCCESS, localize(@"Unable to %@ %@.", skipSubstitutor ? @"restart" : @"start", substitutor->name), skipSubstitutor?false:true);
