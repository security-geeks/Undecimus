...
- (void)viewDidLoad {
    ...
    for (size_t i = 0; exploit_infos[i]; i++) {
        if (!exploit_infos[i]->provides_kernel_task_port) {
            continue;
        }
        NSLog(@"%s", exploit_infos[i]->name);
        [_exploitPickerArray addObject:@(exploit_infos[i]->name)];
        if (!supportsExploit(exploit_infos[i]->exploit)) {
            continue;
        }
    ...
    for (size_t i = 0; substitutor_infos[i]; i++) {
        ...
        if (!supportsSubstitutor(substitutor_infos[i]->substitutor)) {
            continue;
        }
